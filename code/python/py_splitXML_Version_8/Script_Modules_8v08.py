...
def analyze(paramDict):
	...
	for filename in files:
		...
		invalid_tags = ['body', 'text', 'lb', 'TEI','p/'] # Remove a tag only using unwrap()
		for tag in invalid_tags: 
			for match in soup.find_all(tag):
				match.unwrap()
		
		bad_tags = ['teiHeader'] # Remove a tag AND anything within it using decompose()
		for tag in bad_tags:
			for match in soup.find_all(tag):
				match.decompose()

		invalid_hi_tags = ['font0', 'font1', 'font2', 'font3', 'font4', 'font-weight: bold'] # Remove a tag only using unwrap()
		for tag in invalid_hi_tags:
			for match in soup.find_all(rend = tag):
				match.unwrap()
		
		page = soup.prettify(formatter="xml")
...
def cleanAnchor(page):  # LH: If a footnote anchor is within a <hi> tag, for example, take it out of <hi>.
    pattern = re.compile(r'(<hi\s*rend=\"[\w\s]*\">|<hi\s*style=\"\w*\">)([\w\s\|\.,\:\[\]\?\!;\"\"\'\@\#\$%\^\&\*\(\)\-\—\_\+\=\<\>\\\{\}~\`]*?[^\<\@])?(@{2,3}\s*)(\d{1,2}?\s*)?((\D.*?)*?)(<\/hi>)')
    page_reorderAnchor = pattern.sub(r'\1\2\7\3\1\5\7', page)
    emptyhi = re.compile(r'<hi\s*rend="[\w\s]*">\s*</hi>')
...
def newmoveNotes(page):  # LH: This one handles all patterns.
    ...
    onlyp = SoupStrainer(['p','head','table','list'])
    soupop = BeautifulSoup(page, "lxml-xml", parse_only=onlyp)
    ptextl = soupop.find_all(['p','head','table','list'])[2:(len(soupop.find_all(['p','head','table','list']))+1)]

    #onlyp = SoupStrainer(['p','head'])
    #soupop = BeautifulSoup(page, "lxml-xml", parse_only=onlyp)
    #ptextl = soupop.find_all(['p','head'])[2:(len(soupop.find_all(['p','head']))+1)]
    
    pattern1 = re.compile(r'^\s*@{3}\s*\d*(<hi\s*rend=\"[\w\s]*\">\s*\d{1,2}\s*<\/hi>)*')
    pattern2 = re.compile(r'@{2}\s*\d*(<hi\s*rend=\"[\w\s]*\">\s*\d{1,2}\s*<\/hi>)*')
    pattern3 = re.compile(r'\s*<hi\s*rend=\"[\w\s]*\">\s*\d{1,2}\s*<\/hi>\s*')
    pattern4 = re.compile(r'<p>\s*@{3}\s*\d*(<hi\s*rend=\"[\w\s]*\">\s*\d{1,2}\s*<\/hi>)*')
    ...
    if checkfnl.count(1) < 1:  # LH: No footnote moving needed
        ...
        for fn_p in fn_p_l:
            if type(fn_p) == list:
                ...
                fn_str = fn_p.prettify(formatter="xml")    
...
# set directory path to match directory containing processed files
	...
	docx_target_DTD_three = r'<\?xml version="1.0" encoding="utf-8"\?>(<p>)?'
	...
	for file in glob.glob('*.xml'):															# Declare operation as applicable to XML files only - avoids involvement with "Entry_Stats" file
		...
		filedata = re.sub(docx_target_DTD_three,empty,filedata)
		...
		##################### Apr. 8, 2019 ##################################################
		'''
		This block deals with three problems:
		(1) In a <label> tag, if there is any trailing punctuation, move the punctuation out of <label>
		(2) If a trailing punctuation is in a <hi> tag, move the punctuation out of <label>, too
		(3) Clean up ampersand. See below for the specific problematic patterns.  
		'''

		pattern_label = re.compile(r'([\|\.,\:\[\]\?\!;\"\"\'\@\#\$%\^\&\*\(\)\-\—\_\+\=\<\\\{\}~\`])(</label>)')
		filedata = pattern_label.sub(r'\2\1', filedata)
		pattern2_label = re.compile(r'([\|\.,\:\[\]\?\!;\"\"\'\@\#\$%\^\&\*\(\)\-\—\_\+\=\<\\\{\}~\`])(</hi></label>)')
		filedata = pattern2_label.sub(r'\2\1', filedata)

		filedata = re.sub("&amp;amp;lt;","&lt;",filedata)
		filedata = re.sub("&amp;amp;gt;","&gt;",filedata)
		filedata = re.sub("&amp;amp;amp;","&amp;",filedata)        
		filedata = re.sub("amp;lt;","&lt;",filedata)
		filedata = re.sub("amp;gt;","&gt;",filedata)
		filedata = re.sub("amp;amp;","&amp;",filedata)
		filedata = re.sub("&&amp;#8212;&&amp;#8212;","&#8212;&#8212;",filedata)
		filedata = re.sub("&&gt;","&gt;",filedata)
		filedata = re.sub("&&lt;","&lt;",filedata)
