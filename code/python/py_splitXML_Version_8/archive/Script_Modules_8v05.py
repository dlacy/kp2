...
#	page = page.replace("<br clear=\"all\"/>","")					# Removes 'clear=all' break; commented out by pl 3/21/19
...
# set directory path to match directory containing processed files
    ...
    docx_target_DTD_two = ' <name> unknown author </name> </change> </listChange> </revisionDesc><p>'	# Concatenate text and variables into target string
    ...
    for file in glob.glob('*.xml'):															# Declare operation as applicable to XML files only - avoids involvement with "Entry_Stats" file
        ...
        filedata = re.sub("(?<=<hi rend=\"italic\">)\s+|(?<=<hi rend=\"smallcaps\">)\s+|(?<=<hi rend=\"strikethrough\">)\s+|(?<=<hi rend=\"subscript\">)\s+|(?<=<hi rend=\"superscript\">)\s+|(?<=<hi rend=\"underline\">)\s+|(?<=<hi rend=\"italic\"/>)\s+|(?<=<hi rend=\"smallcaps\"/>)\s+|(?<=<hi rend=\"strikethrough\"/>)\s+|(?<=<hi rend=\"subscript\"/>)\s+|(?<=<hi rend=\"superscript\"/>)\s+|(?<=<hi rend=\"underline\"/>)\s+","",filedata)
