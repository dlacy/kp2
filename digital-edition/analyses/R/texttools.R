...
#take corpus and clean texts in it for running in TM
clean.corpus.tm<-function(corpus){
  #for 1 to the length of the corpus..
  for (i in 1:length(corpus)){
    cleaned.text<-text.clean(corpus[[i]])
    new.corpus<-c(new.corpus, cleaned.text)
    #corpus[i]<-cleaned.text artifact from non-TM version hanging on to for time being
  }
  new.tm.corpus<-Corpus(VectorSource(new.corpus))
  return(new.tm.corpus)
}